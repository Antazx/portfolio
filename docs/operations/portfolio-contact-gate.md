...
production, control de abuso externo/distribuido, privacidad aprobada y
aprobación explícita.

`PORTFOLIO_CONTACT_RATE_LIMIT_CONFIGURED=true` no basta: producción también
requiere `PORTFOLIO_CONTACT_RATE_LIMIT_PROVIDER=external` y un adaptador
`distributed` inyectado en la Function. El limiter en memoria solo se admite
en `test`; si falta cualquiera de los dos controles, la Function responde
`503 contact_rate_limit_not_configured` antes de llamar a Brevo.
...
- control externo/distribuido real de cinco solicitudes por diez minutos e IP
  verificado en la plataforma, sin usar el limiter en memoria como protección
  de producción;
...
la petición. Un timeout o una respuesta ambigua devuelve el mensaje localizado
de entrega no confirmada y un enlace `mailto:` al destino personal configurado.
No incluye el mensaje en la URL ni dispara un reenvío automático. La respuesta
JSON, la mejora JavaScript y el HTML nativo muestran ese fallback.
